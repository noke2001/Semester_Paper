...
class SMACSampler(optunahub.samplers.SimpleBaseSampler):
    ...
    def __init__(
        ...
        *,
        ...
        # --- FIX: Initialize the dictionary to track trials ---
        # This fixes the AttributeError in 'after_trial'
        self._trial_info = {} 
        # ----------------------------------------------------
    ...
    def after_trial(
        ...
        if trial.number not in self._trial_info:
            return

        # --- FIX: Handle Failed/Pruned Trials Gracefully ---
        if state in [optuna.trial.TrialState.FAIL, optuna.trial.TrialState.PRUNED]:
            trial_info = self._trial_info[trial.number]
            from smac.runhistory.enumerations import StatusType
            self.smac.tell(
                trial_info,
                trial_value=None, 
                state=StatusType.CRASHED, # Tell SMAC it crashed
            )
            return
        # ---------------------------------------------------

        assert values is not None, "Values must not be None for successful trials."
